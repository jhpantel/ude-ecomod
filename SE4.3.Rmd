...
Our focal summary statistics are (1) the mean of the dataset and (2) the standard deviation of the dataset. We save those to a new variable called `stat.obs`:

```{r tidy=TRUE, tidy.opts=list(width.cutoff=60), warning=FALSE,message=FALSE}
stat.obs <- c(mean(dat$weight_g),sd(dat$weight_g))
```

Our simulated summary statistics can be obtained by calculating the mean and sd for each of the 10000 simulations:

```{r tidy=TRUE, tidy.opts=list(width.cutoff=60), warning=FALSE,message=FALSE}
stat.sim <- array(NA,dim=c(10000,2),dimnames=list(NULL,c("mean","sd")))
for(i in 1:10000){
...
$$ N_{1,t+1} = \frac{N_{1,t} \lambda_{1}}{1 + \alpha_{11} N_{1,t} + \alpha_{12} N_{2,t}}$$
$$ N_{2,t+1} = \frac{N_{2,t} \lambda_{2}}{1 + \alpha_{22} N_{2,t} + \alpha_{21} N_{1,t}}$$
...
disc_lv <- function(lambda_1,lambda_2,N1_0,N2_0,alpha_11,alpha_22,alpha_12,alpha_21){
  Nt1 <- (N1_0*lambda_1) / (1 + alpha_11*N1_0 + alpha_12*N2_0)
  Nt2 <- (N2_0*lambda_2) / (1 + alpha_22*N2_0 + alpha_21*N1_0)
...
# Plot in ggplot
ggplot2::ggplot(N.g,ggplot2::aes(x=time,y=N,col=as.factor(lambda))) + ggplot2::geom_line()
```
...
rand_disc_log <- function(N1_0=N1_0,disc_log,t){
  ...
  alpha_11r <- runif(1,min=-.5,max=.5) # Random uniform distribution, bounded -0.5 - 0.5
...
```{r eval=FALSE}
z <- rbinom(1,1,0.5)
alpha <- z* -rbeta(1,shape1=1,shape2=50) + (1-z) * rbeta(1,shape1=1,shape2=50)
```
